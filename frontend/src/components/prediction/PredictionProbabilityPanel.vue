<template>
  <section class="prob-section" aria-label="预测概率分布">
    ...
    <p v-if="leadingSummary" class="leading-summary" :class="{ 'is-caution': warning }">{{ leadingSummary }}</p>
    <div class="prob-bars">
...
const leadingSummary = computed(() => {
  const leading = items.value.filter(item => item.isLeading)
  if (!leading.length) return ''
  if (leading.length > 1) {
    return `当前最高概率并列：${leading.map(item => `${item.label} ${item.text}`).join(' / ')}`
  }
  const top = leading[0]
  return props.warning
    ? `当前倾向：${top.label}（${top.text}），数据或模型质量不足，仅供参考`
    : `当前倾向：${top.label}（${top.text}）`
})
</script>

<style scoped>
.prob-section { margin-bottom:20px; }
.prob-header { display:flex; justify-content:space-between; align-items:center; margin-bottom:12px; font-size:14px; color:var(--ff-text-muted); gap:12px; }
.prob-hint { color:var(--ff-text-muted); font-size:12px; text-align:right; }
.leading-summary {
  margin: 0 0 14px;
  padding: 10px 12px;
...
.leading-summary.is-caution {
  border-color: rgba(178, 122, 24, 0.45);
  background: #f8f1e4;
  color: #7a5614;
}
...
@media (max-width:768px) {
  ...
  .prob-header { flex-direction:column; align-items:flex-start; }
  .prob-hint { text-align:left; }
}
</style>
