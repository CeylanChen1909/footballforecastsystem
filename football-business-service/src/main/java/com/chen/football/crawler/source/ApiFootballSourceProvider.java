...
@Slf4j
@Component
public class ApiFootballSourceProvider implements MatchSourceProvider {

    private final ApiFootballClient client;
    private final ApiFootballProperties props;

    public ApiFootballSourceProvider(ApiFootballClient client, ApiFootballProperties props) {
        this.client = client;
        this.props = props;
    }

    @Override
    public String name() { return "api-football"; }
    ...
    public boolean isAvailable() {
        return client != null && props.getApiKey() != null && !props.getApiKey().isBlank();
    }
    ...
    public FetchResult fetchMatches(String date) {
        ...
        for (int attempt = 1; attempt <= 3; attempt++) {
            try {
                Map<String, Object> raw = client.getFixturesByDate(date).block();
                FetchResult result = toResult(raw, start);
                if (result.success()) {
                    return result;
                }
                lastEx = new RuntimeException(result.error());
                if (isNonRetryable(result.error())) {
                    log.warn("[ApiFootball] fetchMatches 遇到不可重试错误，停止重试: {}", result.error());
                    return result;
                }
            } catch (Exception e) {
                lastEx = e;
                log.warn("[ApiFootball] fetchMatches 第 {}/3 次尝试失败: {}", attempt, e.getMessage());
                if (isNonRetryable(e.getMessage())) {
                    log.warn("[ApiFootball] fetchMatches 遇到不可重试错误，停止重试: {}", e.getMessage());
                    return FetchResult.failure(name(), e.getMessage(), System.currentTimeMillis() - start);
                }
                if (attempt < 3) sleepQuietly();
            }
        }
        log.warn("[ApiFootball] fetchMatches 重试 3 次仍失败: {}", lastEx.getMessage());
        return FetchResult.failure(name(), lastEx.getMessage(), System.currentTimeMillis() - start);
    }
    ...
    public FetchResult fetchMatchesByLeague(int leagueId, int season) {
        ...
        for (int attempt = 1; attempt <= 3; attempt++) {
            try {
                Map<String, Object> raw = client.getFixtures(leagueId, season, null).block();
                FetchResult result = toResult(raw, start);
                if (result.success()) {
                    return result;
                }
                lastEx = new RuntimeException(result.error());
                if (isNonRetryable(result.error())) {
                    log.warn("[ApiFootball] fetchMatchesByLeague 遇到不可重试错误，停止重试: {}", result.error());
                    return result;
                }
            } catch (Exception e) {
                lastEx = e;
                log.warn("[ApiFootball] fetchMatchesByLeague 第 {}/3 次尝试失败: {}", attempt, e.getMessage());
                if (isNonRetryable(e.getMessage())) {
                    log.warn("[ApiFootball] fetchMatchesByLeague 遇到不可重试错误，停止重试: {}", e.getMessage());
                    return FetchResult.failure(name(), e.getMessage(), System.currentTimeMillis() - start);
                }
                if (attempt < 3) sleepQuietly();
            }
        }
        log.warn("[ApiFootball] fetchMatchesByLeague 重试 3 次仍失败: {}", lastEx.getMessage());
        return FetchResult.failure(name(), lastEx.getMessage(), System.currentTimeMillis() - start);
    }
    ...
    private boolean isNonRetryable(String error) {
    ...
    private FetchResult toResult(Map<String, Object> raw, long start) {
        ...
        if (errors instanceof Map<?, ?> errorMap && !errorMap.isEmpty()) {
            return FetchResult.failure(name(), errorMap.toString(), System.currentTimeMillis() - start);
        }
        if (errors instanceof String errorText && !errorText.isBlank()) {
            return FetchResult.failure(name(), errorText, System.currentTimeMillis() - start);
        }
    ...
    private NormalizedMatch convert(Map<String, Object> item) {
        try {
            ...
            log.debug("[ApiFootball] convert failed: {}", e.getMessage());
            return null;
        }
    }
