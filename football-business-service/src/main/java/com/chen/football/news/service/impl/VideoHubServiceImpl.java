...
public class VideoHubServiceImpl implements VideoHubService {
    ...
    public Map<String, Object> saveVideo(VideoHubItem item, Long userId, String username) {
        ...
        boolean demotedInvalidPublish = false;
        ...
        if ("PUBLISHED".equalsIgnoreCase(item.getStatus()) && !isPlayableVideo(item)) {
            item.setStatus("DRAFT");
            demotedInvalidPublish = true;
        }
        if (isNew) {
        ...
        return Map.of("ok", affected > 0, "id", item.getId(), "isNew", isNew, "affected", affected,
                ...
                "message", demotedInvalidPublish ? "播放地址无效，已保存为草稿；补充真实播放链接后再发布" : "视频已保存");
    }
    ...
    public List<VideoHubItem> listPublicVideos(String keyword, String leagueName, String platform, String videoType, Integer limit) {
        int l = limit == null || limit < 1 ? 20 : Math.min(limit, 1000);
        ...
        return rows == null ? List.of() : rows.stream().filter(this::isPlayableVideo).toList();
    }

    @Override
    public VideoHubPage listPublicVideosPage(String keyword, String leagueName, String platform,
                                             String videoType, Integer page, Integer size) {
        int p = page == null || page < 1 ? 1 : page;
        int s = size == null || size < 1 ? 20 : Math.min(size, 1000);
        Page<VideoHubItem> result = new Page<>(p, s);
        var wrapper = publicWrapper(keyword, leagueName, platform, videoType)
                .orderByDesc(VideoHubItem::getIsFeatured)
                .orderByDesc(VideoHubItem::getIsHot)
                .orderByDesc(VideoHubItem::getSortOrder)
                .orderByDesc(VideoHubItem::getCreatedAt);
        Page<VideoHubItem> rows = videoHubItemMapper.selectPage(result, wrapper);
        List<VideoHubItem> playable = rows.getRecords().stream().filter(this::isPlayableVideo).toList();
        return new VideoHubPage(playable, rows.getTotal(), (int) rows.getCurrent(), (int) rows.getSize());
    }

    @Override
    public long countPublicVideos(String keyword, String leagueName, String platform, String videoType) {
        return videoHubItemMapper.selectCount(publicWrapper(keyword, leagueName, platform, videoType));
    }

    private LambdaQueryWrapper<VideoHubItem> publicWrapper(String keyword, String leagueName,
                                                            String platform, String videoType) {
        return new LambdaQueryWrapper<VideoHubItem>()
                ...
                .eq(StringUtils.hasText(platform), VideoHubItem::getPlatform, platform)
                .eq(StringUtils.hasText(videoType), VideoHubItem::getVideoType, videoType)
                .isNotNull(VideoHubItem::getVideoUrl)
                .ne(VideoHubItem::getVideoUrl, "");
    }
    ...
    /** 公开视频必须有可识别的外部播放地址；平台首页或空链接属于运营草稿，不应冒充视频内容。 */
    private boolean isPlayableVideo(VideoHubItem item) {
        if (item == null || !"PUBLISHED".equalsIgnoreCase(item.getStatus())) return false;
        ...
        try {
            ...
            if (path.isEmpty() || "/".equals(path)) {
                return !(host.contains("youtube.com") || host.contains("bilibili.com") || host.contains("qq.com"));
            }
            return true;
        } catch (Exception e) {
            return false;
